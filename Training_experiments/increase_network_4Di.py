...
out_dir = '/ocean/projects/phy210068p/hsu1/Training_results/' + sim + '/' + galaxy + '/' + dim + '_inc_net'
...
def save_roc(roc_path, epsilon_i, epsilon_a):
    with h5py.File(roc_path, 'a') as f:
        if dim + '_ep_i' in f.keys():
            ...
            f.create_dataset(dim + '_ep_i_inc_net', data=epsilon_i)
            f.create_dataset(dim + '_ep_a_inc_net', data=epsilon_a)
